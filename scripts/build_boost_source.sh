...
if [[ "$PLATFORM" == "darwin" ]] && [[ "$ARCH" == "universal" ]]; then
    ...
    ./b2 stage --stagedir=$BOOST_BUILD_DIR --includedir=$BOOST_BUILD_DIR/include toolset=gcc variant=$VARIANT link=static address-model=32 cflags="-Wall -D_REENTRANT -DCLUNIX -fPIC -O3" -a install
fi
