...
if %ERRORLEVEL% NEQ 0 goto :error
b2 stage --stagedir=%BOOST_INSTALL_DIR% --includedir=%BOOST_INSTALL_DIR%\include --layout=system --with-system --with-filesystem --with-regex link=static runtime-link=%runtimelink% threading=multi address-model=%bitness% variant=%variant% runtime-debugging=%debugging% cflags="/Z7 /ZH:SHA_256 /sdl" cxxflags="/Z7 /ZH:SHA_256 /sdl" install
if %ERRORLEVEL% NEQ 0 goto :error
