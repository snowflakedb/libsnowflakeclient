...
echo === %PERL_EXE% Configure %openssl_debug_option% %openssl_target% no-shared enable-fips /ZH:SHA_256 /sdl
%PERL_EXE% Configure %openssl_debug_option% %openssl_target% no-shared enable-fips /ZH:SHA_256 /sdl
if %ERRORLEVEL% NEQ 0 goto :error
nmake clean
if %ERRORLEVEL% NEQ 0 goto :error
nmake
