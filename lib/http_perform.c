...
sf_bool STDCALL http_perform(CURL *curl,
    ...
    long int last_http_code = 0;
    sf_bool allow_json_on_http_error = SF_BOOLEAN_FALSE;
        ...
        if (res != CURLE_OK) {
          ...
          if (res == CURLE_COULDNT_CONNECT && curl_retry_ctx.retry_count <
            ...
            if (curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &http_code) !=
              ...
              if (!retry) {
                  if (json) {
                      allow_json_on_http_error = SF_BOOLEAN_TRUE;
                  } else {
                      char msg[1024];
      ...
      if (chunk_downloader) {
        ...
        *json = snowflake_cJSON_Parse(buffer.buffer);
        if (*json) {
            ...
            char msg[1024];
            sf_sprintf(msg, sizeof(msg), "Unable to parse JSON text response. Http code: [%ld]",
                last_http_code);
