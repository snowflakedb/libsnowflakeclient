...
#define PTHREAD_LOCK_INIT_ERROR_MSG(e, em)                                   \
    ...
    {                                                                        \
    case EAGAIN:                                                             \
        (em) = "System lacked resources to create lock";                     \
        break;                                                               \
    case ENOMEM:                                                             \
        (em) = "Insufficient memory to create mutex";                        \
        break;                                                               \
    case EPERM:                                                              \
        (em) = "Caller doesn't have the privilege to perform the operation"; \
        break;                                                               \
    case EBUSY:                                                              \
        (em) = "Mutex already initialized";                                  \
        break;                                                               \
    case EINVAL:                                                             \
        (em) = "The value specified by attr is invalid";                     \
        break;                                                               \
    default:                                                                 \
        (em) = "Unknown non-zero pthread init error";                        \
        break;                                                               \
    }
...
#define PTHREAD_CREATE_ERROR_MSG(e, em)                                                       \
    ...
    {                                                                                         \
    case EAGAIN:                                                                              \
        (em) = "System lacked resources to create another thread";                            \
        break;                                                                                \
    case EPERM:                                                                               \
        (em) = "Caller doesn't have the privilege to set the required scheduling parameters"; \
        break;                                                                                \
    case EINVAL:                                                                              \
        (em) = "The value specified by attr is invalid";                                      \
        break;                                                                                \
...
#define PTHREAD_JOIN_ERROR_MSG(e, em)                                                      \
    ...
    {                                                                                      \
    case EDEADLK:                                                                          \
        (em) = "A deadlock was detected (i.e. two threads tried to join with each other)"; \
        break;                                                                             \
    case EPERM:                                                                            \
        (em) = "Not a joinable thread";                                                    \
        break;                                                                             \
    case ESRCH:                                                                            \
        (em) = "No thread with specified ID could be found";                               \
        break;                                                                             \
    default:                                                                               \
        (em) = "Unknown non-zero pthread join error";                                      \
        break;                                                                             \
    }
...
static void *chunk_downloader_thread(void *downloader)
{
    ...
    while (1)
    {
        ...
        // Download chunk
        cJSON **chunk_ptr = &chunk;
        NON_JSON_RESP *non_json_resp = NULL;
        // create response buffer for arrow
        if (chunk_downloader->callback_create_resp)
        {
            chunk_ptr = NULL;
                            ...
                            chunk_downloader->network_timeout, chunk_downloader->retry_max_count))
        {
            ...
            _rwlock_wrunlock(&chunk_downloader->attr_lock);
            // Notify the consumer that we have an error
        ...
        if (chunk_downloader->callback_create_resp)
        {
            chunk_downloader->queue[index].chunk = (void *)non_json_resp;
        }
        else
        {
