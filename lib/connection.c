...
sf_bool STDCALL create_header(SF_CONNECT *sf, SF_HEADER *header, SF_ERROR_STRUCT *error)
{
    ...
    header->header = curl_slist_append(header->header,
                                       header->use_application_json_accept_type ? HEADER_ACCEPT_TYPE_APPLICATION_JSON : HEADER_ACCEPT_TYPE_APPLICATION_SNOWFLAKE);
...
DECORRELATE_JITTER_BACKOFF *
    STDCALL
    decorrelate_jitter_init(uint32 base, uint32 cap)
{
                         ...
                         char *extraUrlParams)
{
    ...
    encoded_url_size += extraUrlParams ? strlen(extraUrlParams) + strlen(URL_PARAM_DELIM) : 0;
