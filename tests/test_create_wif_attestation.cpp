...
public:
  explicit FakeHttpClient(std::function<HttpResponse(HttpRequest)> run_fn)
    : run_fn(std::move(run_fn)) {}
  Snowflake::Client::HttpResponse run(HttpRequest req) override {
    return run_fn(req);
  }
private:
  std::function<HttpResponse(HttpRequest)> run_fn;
};
