                          ...
                          bool native = false)
{
  ...
  if (!native)
  {
    if (testUnicode)
    {
      stmtPutGet = std::unique_ptr<StatementPutGetUnicode>(new Snowflake::Client::StatementPutGetUnicode(sfstmt));
    }
    else
    {
      stmtPutGet = std::unique_ptr<StatementPutGet>(new Snowflake::Client::StatementPutGet(sfstmt));
    }
  }
  ...
  std::string expectedTarget = (autoCompress && !strstr(expectedSrc.c_str(), ".gz")) ? expectedSrc + ".gz" : expectedSrc;
