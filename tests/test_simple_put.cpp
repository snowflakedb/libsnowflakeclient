...
{
    // Skip this test when CLIENT_CODE_COVERAGE=1
    ...
    if (codecovenv && strlen(codecovenv) > 0 && githubenv && strlen(githubenv) > 0)
    {
      if (!strncmp(codecovenv, "1", 1)) {
        std::cout << "Not running test_large_reupload because it fails on linux when CLIENT_CODE_COVERAGE=1" << std::endl;
        errno = 0;
        return;
      }
    }
...
void test_verify_upload(void **unused)
{
    ...
    {
        if (!strncmp(codecovenv, "1", 1)) {
            std::cout << "Not running test_verify_upload because it fails on linux when CLIENT_CODE_COVERAGE=1" << std::endl;
            errno = 0;
            return;
        }
    }
...
void test_large_get(void **unused)
{
  ...
  {
    if (!strncmp(codecovenv, "1", 1)) {
      std::cout << "Not running test_large_get because it fails on linux when CLIENT_CODE_COVERAGE=1" << std::endl;
      errno = 0;
      return;
    }
  }
...
void test_large_get_threshold(void **unused)
{
  ...
  {
    if (!strncmp(codecovenv, "1", 1)) {
      std::cout << "Not running test_large_get_threshold because it fails on linux when CLIENT_CODE_COVERAGE=1" << std::endl;
      errno = 0;
      return;
    }
  }
