#include <string.h>
#include "utils/test_setup.h"
#include "connection.h"
#include "./authenticator.h"
#include "memory.h"

/*
 * Test authenticator workflow.
 */
void test_unit_connect(void** unused) 
{
    SF_UNUSED(unused);
    SF_CONNECT *sf = snowflake_init();
    snowflake_set_attribute(sf, SF_CON_ACCOUNT,"test_account");
    snowflake_set_attribute(sf, SF_CON_USER, "test_user");
    snowflake_set_attribute(sf, SF_CON_HOST, "host");
    snowflake_set_attribute(sf, SF_CON_PORT, "443");
    snowflake_set_attribute(sf, SF_CON_PROTOCOL, "https");
    snowflake_set_attribute(sf, SF_CON_AUTHENTICATOR, "test");

    auth_initialize(sf);
    auth_authenticate(sf);
    cJSON* body = snowflake_cJSON_CreateObject();

    auth_update_json_body(sf, body);
    cJSON* data = snowflake_cJSON_GetObjectItem(body, "data");

    assert_int_equal(snowflake_cJSON_GetNumberValue(snowflake_cJSON_GetObjectItem(data, "test")), 0);

    auth_renew_json_body(sf, body);
    data = snowflake_cJSON_GetObjectItem(body, "data");
    assert_int_equal(snowflake_cJSON_GetNumberValue(snowflake_cJSON_GetObjectItem(data, "test")), 1);

    snowflake_cJSON_Delete(body);
    snowflake_term(sf);
}

void test_heart_beat_connection_parameters(void** unused)
{
    SF_UNUSED(unused);
    SF_CONNECT* sf = snowflake_init();
    void* value = NULL;
    sf_bool keepAlive = SF_BOOLEAN_FALSE;
    snowflake_set_attribute(sf, SF_CON_CLIENT_SESSION_KEEP_ALIVE, &keepAlive);
    snowflake_get_attribute(sf, SF_CON_CLIENT_SESSION_KEEP_ALIVE, &value);
    assert_int_equal(*((sf_bool*)value), keepAlive);

    uint64 heart_beat_interval = 10000;
    snowflake_set_attribute(sf, SF_CON_CLIENT_SESSION_KEEP_ALIVE_HEARTBEAT_FREQUENCY, &heart_beat_interval);
    snowflake_get_attribute(sf, SF_CON_CLIENT_SESSION_KEEP_ALIVE_HEARTBEAT_FREQUENCY, &value);
    assert_int_equal(*((uint64*)value), 3600);

    heart_beat_interval = 10;
    snowflake_get_attribute(sf, SF_CON_CLIENT_SESSION_KEEP_ALIVE_HEARTBEAT_FREQUENCY, &value);
    snowflake_set_attribute(sf, SF_CON_CLIENT_SESSION_KEEP_ALIVE_HEARTBEAT_FREQUENCY, &heart_beat_interval);
    assert_int_equal(*((uint64*)value), 900);

    heart_beat_interval = 1234;
    snowflake_set_attribute(sf, SF_CON_CLIENT_SESSION_KEEP_ALIVE_HEARTBEAT_FREQUENCY, &heart_beat_interval);
    snowflake_set_attribute(sf, SF_CON_CLIENT_SESSION_KEEP_ALIVE_HEARTBEAT_FREQUENCY, &heart_beat_interval);
    assert_int_equal(*((uint64*)value), heart_beat_interval);
}

int main(void) 
{
    initialize_test(SF_BOOLEAN_FALSE);
    const struct CMUnitTest tests[] = {
      cmocka_unit_test(test_unit_connect),
      cmocka_unit_test(test_heart_beat_connection_parameters),
    };
    int ret = cmocka_run_group_tests(tests, NULL, NULL);
    snowflake_global_term();
    return ret;
}
