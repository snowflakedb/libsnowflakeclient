...
void Diagnostics::AddError(const DriverException& exception) {
...
void Diagnostics::AddWarning(std::string message, std::string sql_state,
...
std::string Diagnostics::GetMessageText(uint32_t record_index) const {
  std::string message;
  if (!vendor_.empty()) {
