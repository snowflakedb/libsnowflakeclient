...
def check_run(run):
    ...
    return all((
        any((
            ...
            not (first["end_time"] <= second["start_time"]),
        )),
        any((
            second["start_time"] < third["start_time"],
            not (second["end_time"] <= third["start_time"]),
        )),
        any((
            third["start_time"] < first["start_time"],
            not (third["end_time"] <= first["start_time"]),
        )),
    ))
