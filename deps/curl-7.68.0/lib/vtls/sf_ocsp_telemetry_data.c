...
#include "sf_ocsp_telemetrty_data.h"
...
void sf_otd_set_event_sub_type(char *event_sub_type, SF_OTD* ocsp_telemetry_data)
{
  ...
  if (strlen(ocsp_telemetry_data) > 0)
  {
    strncat(ocsp_telemetry_data->event_sub_type, " | ",
        OCSP_RESPONSE_SUB_EVENT_MAX_LEN-strlen(ocsp_telemetry_data->event_sub_type));
...
void sf_otd_set_certid(char *certid, SF_OTD* ocsp_telemetry_data)
{
  ...
  snprintf(ocsp_telemetry_data->certid,OCSP_TELEMETRY_CERTID_MAX_LEN, certid));
}
