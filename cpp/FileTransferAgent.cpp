...
Snowflake::Client::FileTransferAgent::execute(string *command)
{
  ...
  switch (response.command)
  {
    case CommandType::UPLOAD: {
      ...
      CXX_LOG_DEBUG("Time took to upload %s: %ld milli seconds.", getQueryCommand(command), totalTime);
      break;
    }
...
void Snowflake::Client::FileTransferAgent::upload(string *command)
{
  ...
  {
    CXX_LOG_DEBUG("No files to upload, source files do not exist. put command %s FAILED.", getQueryCommand(command));
  ...
  {
    CXX_LOG_DEBUG("%s command FAILED.", getQueryCommand(command));
    if (isPutFastFailEnabled())
    {
...
extern "C" {
                        ...
                        struct SF_QUERY_RESULT_CAPTURE* result_capture)
  {
    ...
    agent.setLogQueryText(sfconn->log_query_text);
