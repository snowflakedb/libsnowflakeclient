...
namespace Snowflake
{
  namespace Client
  {
    ...
    SnowflakeS3Client::SnowflakeS3Client(StageInfo *stageInfo,
                                         ...
                                         IStatementPutGet *statement) : m_stageInfo(stageInfo),
                                                                        ...
                                                                        m_parallel(std::min(parallel, std::thread::hardware_concurrency()))
    {
      ...
      {
        clientConfiguration.endpointOverride = Aws::String("s3.") + Aws::String(clientConfiguration.region) + Aws::String(".") + Aws::String(getDomainSuffixForRegionalUrl(stageInfo->region));
      }
      ...
      if (!proxy.getMachine().empty())
      {
        ...
        clientConfiguration.proxyScheme = proxy.getScheme() == Snowflake::Client::Util::Proxy::Protocol::HTTPS ? Aws::Http::Scheme::HTTPS : Aws::Http::Scheme::HTTP;
        ...
        std::basic_iostream<char> *dataStream)
    {
      ...
      for (size_t i = 0; i < partNum; i++)
      {
        ...
        rangeStream << "bytes=" << i * DOWNLOAD_DATA_SIZE_THRESHOLD << '-' << ((i == partNum - 1) ? fileMetadata->srcFileSize - 1 : ((i + 1) * DOWNLOAD_DATA_SIZE_THRESHOLD - 1));

        downloadParts.emplace_back();
        downloadParts.back().m_partNumber = i;
        downloadParts.back().getObjectRequest.WithBucket(bucket).WithKey(key).WithRange(rangeStream.str());
      }
        ...
        std::basic_iostream<char> *dataStream)
    {
      ...
      std::function<Aws::IOStream *(void)> responseStreamCb = [&]() -> Aws::IOStream *
      {
        // Clear the stream as aws sdk expects a clean stream returned from
        // response stream factory each time when retry on network errors
        dataStream->clear();
