    ...
    namespace Jwt
    {
      std::vector<char> CJSONOperation::serialize(cJSON *root)
      {
        std::unique_ptr<char, std::function<void(char *)>>
            json_str(snowflake_cJSON_Print(root), [](char *str)
                     { snowflake_cJSON_free(str); });

        if (json_str == nullptr)
          throw JwtException("Error serializing JSon object");
      ...
      cJSON *CJSONOperation::parse(const std::vector<char> &text)
      {
        ...
        if (root == nullptr)
          throw JwtException("Error parsing JSon object");
