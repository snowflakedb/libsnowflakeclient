  ...
  namespace Client
  {
    namespace Jwt
    {
      ...
      std::string RSASigner<Hash>::sign(EVP_PKEY *key, const std::string &msg)
      {
        ...
        if (1 != EVP_DigestSignInit(mdctx.get(), nullptr, Hash{}(), nullptr, key))
          return "";
        ...
        size_t slen;
        if (1 != EVP_DigestSignFinal(mdctx.get(), nullptr, &slen))
          return "";
        std::vector<char> buf(slen);
        /* Obtain the signature */
        if (1 != EVP_DigestSignFinal(mdctx.get(), (unsigned char *)buf.data(), &slen))
          return "";
