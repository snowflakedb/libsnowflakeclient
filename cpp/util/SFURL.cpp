...
namespace Snowflake
{
  namespace Client
  {
    ...
    /// Functions of SFURL
    SFURL::SFURL()
        : m_cacheValid(false), m_params(m_cacheValid, m_cacheURL), m_proxyEnabled(false) {}

    SFURL::SFURL(const SFURL &copy)
        : m_cacheURL(copy.m_cacheURL), m_cacheValid(copy.m_cacheValid), m_scheme(copy.m_scheme), m_userinfo(copy.m_userinfo), m_host(copy.m_host), m_port(copy.m_port), m_path(copy.path()), m_params(copy.m_params, m_cacheValid, m_cacheURL), m_fragment(copy.m_fragment), m_proxy(copy.m_proxy), m_proxyEnabled(copy.m_proxyEnabled)
    {
    }
