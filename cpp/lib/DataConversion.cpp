...
)
{
    ...
    sfTimestamp.tm_obj = *(std::localtime(&t));
    sfTimestamp.nsec = ts->fs->Value(cellIdx);
    sfTimestamp.tzoffset = ts->tz->Value(cellIdx);
    sfTimestamp.scale = static_cast<int64>(scale);
    sfTimestamp.ts_type = snowType;

    // Leverage client library to convert to string.
    // The client will handle the allocation of resources if necessary.
    return snowflake_timestamp_to_string(
        &sfTimestamp,
        "",
        out_data,
        *io_capacity,
