...
extern "C"
{
  ...
  SF_STATUS STDCALL auth_initialize(SF_CONNECT *conn)
  {
    ...
    try
    {
      ...
      if (AUTH_USR_PWD_MFA == auth_type)
      {
        conn->auth_object = static_cast<Snowflake::Client::IAuthenticator *>(
            new Snowflake::Client::AuthenticatorUserMFA(conn));
      }
    }
    catch (...)
    {
  ...
  void auth_update_json_body(SF_CONNECT *conn, cJSON *body)
  {
    ...
    try
    {
      static_cast<Snowflake::Client::IAuthenticator *>(conn->auth_object)->updateDataMap(body);
    }
    catch (...)
    {
      ; // Do nothing
  ...
  void auth_renew_json_body(SF_CONNECT *conn, cJSON *body)
  {
    ...
    try
    {
      static_cast<Snowflake::Client::IAuthenticator *>(conn->auth_object)->renewDataMap(body);
    }
    catch (...)
    {
      ; // Do nothing
...
namespace Snowflake
{
  namespace Client
  {
    ...
    {
      std::unique_ptr<EVP_MD_CTX, std::function<void(EVP_MD_CTX *)>> mdctx{EVP_MD_CTX_new(), [](EVP_MD_CTX *ctx)
    ...
    AuthenticatorUserMFA::~AuthenticatorUserMFA()
    {
    }

    void AuthenticatorUserMFA::authenticate()
    {
    }
