...
namespace Snowflake
{
    namespace Client
    {
        namespace IAuth
        {
            ...
            {
                SFURL url = SFURL().scheme(m_protocol).host(m_host).port(m_port);
            ...
            void IAuthenticatorOKTA::authenticate()
            {
                ...
                while (true)
                {
                    ...
                    oneTimeToken = respData.find("sessionToken") != respData.end() ? respData["sessionToken"].get<std::string>() : respData["cookieToken"].get<std::string>();
