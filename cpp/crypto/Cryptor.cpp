  ...
  namespace Client
  {
    namespace Crypto
    {
                                         ...
                                         const CryptoRandomDevice dev)
      {
        ...
        {
          const T rdv = rd();
          sf_memcpy(outT, nbBytes - (outT - out), &rdv, STEP);
          outT += STEP;
        }
        ...
        {
          const T remVal = rd();
          sf_memcpy(outT, nbBytes - (outT - out), &remVal, rem);
        }
      }
...
#define SF_CRYPTO_GET_CIPHER(algo, nbBits)        \
  ...
  {                                               \
  case CryptoMode::CBC:                           \
    return SF_CRYPTO_CIPHER(algo, nbBits, cbc)(); \
  case CryptoMode::CFB:                           \
    return SF_CRYPTO_CIPHER(algo, nbBits, cfb)(); \
  case CryptoMode::ECB:                           \
    return SF_CRYPTO_CIPHER(algo, nbBits, ecb)(); \
  case CryptoMode::GCM:                           \
    return SF_CRYPTO_CIPHER(algo, nbBits, gcm)(); \
  case CryptoMode::OFB:                           \
    return SF_CRYPTO_CIPHER(algo, nbBits, ofb)(); \
  default:                                        \
    break;                                        \
  }                                               \
  break;
