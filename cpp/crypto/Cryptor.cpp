 ...
 */
            /*static void getRandomBytesFromFile(char *const out,
                ...
                static const string DEV_RANDOM_STR("/dev/random");
                static const string DEV_URANDOM_STR("/dev/urandom");
                const string &token = dev == CryptoRandomDevice::DEV_RANDOM
                                           ? DEV_RANDOM_STR
                                           : DEV_URANDOM_STR;

                // Random value type.
                typedef random_device::result_type T;
                static constexpr size_t STEP = sizeof(T);
                const size_t steps = nbBytes / STEP;
                const size_t rem = nbBytes % STEP;

                // Create random device.
                random_device rd(token);
