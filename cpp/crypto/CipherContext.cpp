 ...
 */
            ...
            void CipherContext::initialize(const CryptoOperation op,
                ...
                auto &impl = *m_pimpl;
                /*static_assert(sizeof(impl.iv.data) == sizeof(ub16),
                              "Unexpected initialization vector size");
                const void *const ivData = impl.iv.data;
                ub16 ivInt = unalignedLoad(*static_cast<const ub16 *>(ivData));*/
                /*if (impl->mode == CryptoMode::CTR) {
                //TODO counter mode
                    if (offset) {
                        // Offset must be a multiple of the cipher block size.
                        SF_ASSERT(!(offset % AES_BLOCK_SIZE),
                                  "offset_not_aligned_to_cipher_block_size",
                                  offset % AES_BLOCK_SIZE);

                        // OpenSSL treats the IV as a 128bit number in big endian format. It then
                        // increments this number by 1 for each cipher block worth of data. We
                        // convert the IV to little endian to perform the addition, and then
                        // convert the result back to big endian.
                        ub16 ivIntLe = (ub16{SF_BSWAP_8(ivInt)} << 64) |
                                       SF_BSWAP_8(ivInt >> 64);
                        ivIntLe += offset / AES_BLOCK_SIZE;
                        ivInt = (ub16{SF_BSWAP_8(ivIntLe)} << 64) |
                                SF_BSWAP_8(ivIntLe >> 64);
                    }
                } else {
                    SF_ASSERT(!offset, "offset_requires_ctr_mode",
                              offset % static_cast<int>(impl.mode));
                }*/
