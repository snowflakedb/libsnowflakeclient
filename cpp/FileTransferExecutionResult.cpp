...
namespace Snowflake
{
  namespace Client
  {
    ...
    FileTransferExecutionResult::FileTransferExecutionResult(
        ...
        size_t resultEntryNum) : m_commandType(commandType),
        ...
        RemoteStorageRequestOutcome outcome)
    {
      ...
      {
      case SUCCESS:
        return m_commandType == CommandType::UPLOAD ? STATUS_UPLOADED : STATUS_DOWNLOADED;
      ...
      if (columnIndex < getColumnSize())
      {
        ...
        case UPLOAD:
          ...
          case PUT_COLUMN::SOURCE_COMPRESSION:
            value = std::string(m_fileMetadatas[m_currentIndex]->sourceCompression->getName());
            break;

          case PUT_COLUMN::TARGET_COMPRESSION:
            value = std::string(m_fileMetadatas[m_currentIndex]->targetCompression->getName());
            break;
    ...
    {
      std::vector<const char *> *arraysToFind = m_commandType == UPLOAD ? &PUT_COLUMN_NAMES : &GET_COLUMN_NAMES;
