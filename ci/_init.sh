...
if [[ "$PLATFORM_ARCH" == "aarch64" ]]; then
  ...
  declare -A BUILD_IMAGE_NAMES=(
    [$DRIVER_NAME-$DOCKER_MARK]=client-$DRIVER_NAME-ubuntu20-aarch64:$BUILD_IMAGE_VERSION_AARCH64
  )

  declare -A TEST_IMAGE_NAMES=(
    [$DRIVER_NAME-$DOCKER_MARK]=client-$DRIVER_NAME-ubuntu20-aarch64:$TEST_IMAGE_VERSION_AARCH64
  )
else
